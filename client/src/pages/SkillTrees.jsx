...
export default function SkillTrees() {
  ...
  useEffect(() => {
    skillTreeService.list().then(r => { setTrees(r.data.data.skillTrees); setLoading(false); }).catch(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (selected) {
      skillTreeService.get(selected._id).then(r => setDetail(r.data.data.skillTree)).catch(() => {});
    }
  }, [selected]);
      ...
      <AnimatePresence mode="wait">
        {!selected ? (
            ...
            {trees.map((tree, i) => (
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <div className="flex justify-between items-center mt-lg">
                    <span className="badge" style={{ background: `${tree.color}20`, color: tree.color, border: `1px solid ${tree.color}40` }}>
                      {tree.nodes?.length || 0} Mastery Nodes
                    </span>
                    <motion.div 
                ...
                <div style={{ position: 'absolute', bottom: 0, left: 0, height: '3px', width: '100%', background: 'rgba(255,255,255,0.05)' }}>
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.random() * 60 + 20}%` }} // Mock progress for visual wow
          ...
          >
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden', border: `1px solid ${selected.color}40`, boxShadow: `0 20px 50px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.05)` }}>
              ...
              <div style={{ padding: '2rem', background: `linear-gradient(to right, rgba(0,0,0,0.8), rgba(0,0,0,0.4)), ${selected.color}20`, position: 'relative' }}>
                <div className="flex items-center gap-lg position-relative z-10">
                  <motion.div 
                    ...
                    style={{ fontSize: '3.5rem', filter: `drop-shadow(0 0 20px ${selected.color})` }}
                  >
                    {selected.icon}
                  </motion.div>
                  <div>
                    <h2 style={{ fontSize: '2.5rem', marginBottom: '0.5rem', color: 'var(--text-primary)', textShadow: `0 0 20px ${selected.color}80` }}>{selected.name}</h2>
              ...
              <div style={{ 
                ...
                <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1 }}>
                  {detail?.nodes?.map(node => node.connections?.map(connId => {
                    const target = detail.nodes.find(n => n.nodeId === connId);
                      ...
                      <g key={`${node.nodeId}-${connId}`}>
                        ...
                        {isUnlockedPath && (
                          <motion.line 
                            ...
                            style={{ filter: `drop-shadow(0 0 5px ${selected.color})` }}
                          />
                        )}
                      </g>
                    );
                      ...
                      <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto 0.75rem' }}>
                        ...
                        {isUnlocked && !isCompleted && (
                          <motion.div
                            ...
                            style={{ position: 'absolute', inset: -10, background: selected.color, borderRadius: '50%', filter: 'blur(10px)', zIndex: 0 }}
                          />
                        )}
                        ...
                        <svg width="80" height="80" viewBox="0 0 80 80" style={{ position: 'absolute', top: 0, left: 0, transform: 'rotate(-90deg)', zIndex: 1 }}>
                          ...
                          <motion.circle cx="40" cy="40" r="36" fill="none" stroke={selected.color} strokeWidth="6" strokeLinecap="round"
                            ...
                            style={{ filter: `drop-shadow(0 0 4px ${selected.color})` }}
                          />
                        </svg>
                        ...
                        <div style={{
                          ...
                          boxShadow: isCompleted ? `0 0 20px ${selected.color}, inset 0 0 10px rgba(255,255,255,0.5)` : 'inset 0 2px 10px rgba(0,0,0,0.5)',
                        ...
                        {hoveredNode === node.nodeId && (
                          <motion.div
                            ...
                            style={{
                              ...
                              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 20px ${selected.color}20`,
